...
function rate_conflict(){
    document.querySelectorAll('.rate_conflict').forEach(card => {
        const rating = parseFloat(card.querySelector('.number-rate').textContent);
        const starContainer = card.querySelector('.star-container');
        updateStars(starContainer, rating);
    });
...
//     alertBox.className = '';
//     alertBox.classList.add(stateClass);
    
//     // إعداد محتوى الرسالة مع الأيقونة
//     alertBox.innerHTML = `
...
//         // مسح المحتوى بعد انتهاء الانتقال
//         setTimeout(() => {
//             alertBox.innerHTML = '';
//         }, 500);
//     }, 3000);
...
//     // إذا كانت الاستجابة تحتوي على كائن كامل
//     if (typeof state === 'object' && state !== null) {
//         message = state.message || message;
//         state = state.status || 'info';
//     }
...
function showAlert(state, message) {
    ...
    setTimeout(() => {
        ...
        // مسح المحتوى وإزالة الكلاسات بعد انتهاء الانتقال
        setTimeout(() => {
            alertBox.innerHTML = '';
            alertBox.className = ''; 
            alertBox.classList.add('hidden');
            // إزالة جميع الكلاسات المضافة
        }, 500);
    }, 3000);
}